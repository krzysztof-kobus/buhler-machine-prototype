...
@Component({
  ...
  template: `
    ...
    >
      <span class="machine-btn__state-icon material-symbols-outlined" aria-hidden="true">{{
        stateIcon(state())
      }}</span>
      @if (!small()) {
        <span class="machine-btn__machine-icon material-symbols-outlined" aria-hidden="true">{{
          machineIcon()
        }}</span>
      }
